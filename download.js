...
const baseURL = 'http://www.effectgames.com/demos/canvascycle/';

const sceneURLs = [];

for (const scene of scenes) {
  // Generate URLs for image, .ogg, and .mp3
  const imageURL = `${baseURL}image.php?file=${scene.name}&callback=CanvasCycle.processImage`;
  const oggURL = `${baseURL}audio/${scene.sound}.ogg`;
  const mp3URL = `${baseURL}audio/${scene.sound}.mp3`;

  sceneURLs.push(imageURL);
  sceneURLs.push(oggURL);
  sceneURLs.push(mp3URL);
}

// Write the list of URLs to a text file
const outputFileName = 'sceneURLs.txt';

fs.writeFile(outputFileName, sceneURLs.join('\n'), (err) => {
  if (err) {
    console.error('Error writing file:', err);
  } else {
    console.log(`URLs written to ${outputFileName}`);
  }
});
